...
interface CreateGroupMutationVars {
  accountId: string
  name: string
}

interface LeaveGroupMutationVars {
  accountId: string
  groupId: string
}
...
export default function Groups(): React.ReactElement {
  ...
  // WA accounts + groups
  const { data: waAccounts = [] } = useQuery({ queryKey: ['waAccounts'], queryFn: getWAAccounts, retry: false }) as { data: WAAccount[] }
  ...
  // TG chats
  const { data: tgChats = [] } = useQuery({ queryKey: ['tgChats'], queryFn: () => getTGChats(true), retry: false }) as { data: TGChat[] }

  // Flatten all WA groups
  const allWAGroups = waGroupQueries.flatMap((q: any) =>
    (q.data || []).map((g: any) => ({ ...g, accountId: q.account.id, accountName: q.account.name, provider: 'whatsapp' }))
  )

  const createGroup = useMutation({
    mutationFn: ({ accountId, name }: CreateGroupMutationVars) => createWAAccountGroup(accountId, name),
    onSuccess: (_, { accountId }: CreateGroupMutationVars) => {
      qc.invalidateQueries({ queryKey: ['waGroups', accountId] })
      setNewGroupName('')
    },
  })
  const leaveGroup = useMutation({
    mutationFn: ({ accountId, groupId }: LeaveGroupMutationVars) => leaveWAAccountGroup(accountId, groupId),
    onSuccess: (_, { accountId }: LeaveGroupMutationVars) => qc.invalidateQueries({ queryKey: ['waGroups', accountId] }),
  })
    ...
    <div>
      ...
      {connectedAccounts.length > 0 && (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-6">
          ...
          {createGroup.isError && <p className="text-xs text-red-400 mt-2">{(createGroup.error as any)?.response?.data?.detail || 'Erro'}</p>}
        </div>
      )}
