...
export default function Channels(): React.ReactElement {
  ...
  const [newForm, setNewForm] = useState<NewChannelForm>({ name: '', description: '' })

  const { data: channels = [], isLoading } = useQuery({ queryKey: ['channels'], queryFn: getChannels }) as { data: Channel[], isLoading: boolean }
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-1 mt-2 flex-wrap">
                  ...
                  {(ch.rules?.length ?? 0) > 0 && (
                    <span className="text-xs text-gray-500 ml-2">{ch.rules?.length} regra{(ch.rules?.length ?? 0) > 1 ? 's' : ''}</span>
                  )}
                  {(ch.sent_count ?? 0) > 0 && (
                    <span className="text-xs text-gray-500 ml-2">📤 {ch.sent_count} enviada{(ch.sent_count ?? 0) > 1 ? 's' : ''}</span>
                  )}
                </div>
              </div>
